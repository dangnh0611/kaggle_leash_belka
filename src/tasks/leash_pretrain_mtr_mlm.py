...
class LeashPretrainJoinMTRMLMTask(BaseTask):
    ...
    def _shared_step(self,
        ...
        mlm_target = batch['mlm_target']
        mtr_pred, mlm_pred = model(batch['input_ids'], batch['padding_mask'])
        mtr_loss, mlm_loss, total_loss = self.criterion(
            mtr_pred, mtr_target, mlm_pred, mlm_target)
        step_metrics = {
            f"{stage}/mtr_loss": mtr_loss,
            f"{stage}/mlm_loss": mlm_loss,
            f"{stage}/loss": total_loss,
        }
        step_output = {"loss": total_loss}

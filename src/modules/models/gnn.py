...
class GNNModel(nn.Module):
    ...
    """
        ...
        if cfg.pretrain_ckpt is not None:
            self.load_pretrained(cfg.pretrain_ckpt)

    def load_pretrained(self, model_file):
        state_dict = torch.load(model_file)
        logger.info('Load pretrain model from %s', model_file)
        logger.info('Model params: %s\nPretrain params:%s',
                    self.gnn.state_dict().keys(), state_dict.keys())
